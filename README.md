...
- Throughout the repository, the name `g1_exp_variable_frequency` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of g1_exp_variable_frequency (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e exts/g1_exp_variable_frequency
```
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/exts/g1_exp_variable_frequency"
    ]
}
```
